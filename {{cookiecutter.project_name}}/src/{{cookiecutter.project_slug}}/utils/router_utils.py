...
from {{cookiecutter.project_slug}}.models.error_codes import UNEXPECTED_ERROR
from {{cookiecutter.project_slug}}.models.error_response import ApiErrorException

T = TypeVar("T")
logger = get_logger("router_utils")


def handle_exception(exp: Exception, request_id: str | None = None) -> None:
    """Handle an exception by logging and raising an API error."""
...
def handle_result(result: Result[T, Exception], request_id: str | None = None) -> T:
    ...
    Raises:
        ApiErrorException: If the Result contains an error
    """
    log_context = {"request_id": request_id} if request_id else {}
